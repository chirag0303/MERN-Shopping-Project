    ...
    const handleEditProduct = async (productId) => {
        try {
            ...
            if (res.status === 200) {
                ...
                alert("Error while updating value", result.message);
            }
    ...
    return (
        <div>
            ...
            <div className="flex flex-wrap gap-6 justify-center p-3 ">
                {products.map((elem) => {
                    return (
                        <div key={elem._id} className="p-4 rounded-lg border-1 bg-yellow-200">
                            <p className="font-bold">{elem.title}</p>
                            {elem._id === editProductId ?
                                <>
                                    <div className="flex gap-1">
                                        <input value={updatedPrice} onChange={(e) => setUpdatedPrice(e.target.value)} className="border-1 py-1 px-2 rounded-md" size="7" />
                                        <button className="bg-red-400 text-white px-2 py-0.3 rounded hover:bg-red-500 transition duration-200" onClick={() => setEditProductId("")}>Cancel</button>
                                        <button className="bg-green-500 text-white px-2 py-0.3 rounded hover:bg-green-600 transition duration-200" onClick={() => handleEditProduct(elem._id)}>Save</button>
                                    </div>
                                ...
                                <div >
                                    ...
                                    <p>{elem.description}</p>
                                    <button onClick={() => { setEditProductId(elem._id); setUpdatedPrice(elem.price) }} className="bg-blue-400 text-white px-2 py-1 rounded hover:bg-blue-500 transition duration-200 mr-2">Edit</button>
                                    <button onClick={() => { setCount((prev) => prev + 1) }} className="bg-blue-400 text-white px-2 py-1 mt-1 rounded hover:bg-blue-500 transition duration-200">Add to Cart</button>
                                </div>
                            }
